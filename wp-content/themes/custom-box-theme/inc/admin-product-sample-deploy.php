...
function custom_box_product_sample_deploy_batch_complete( string $marker, int $expected_count, int $min_words = 1500, string $expected_moq = '1000 boxes', string $expected_status = 'publish', int $expected_specs = 21 ): bool {
	...
	foreach ( $products as $product ) {
		...
		if ( ! $status_ok || $words < $min_words || preg_match( '/<h1\b/i', $content ) || ! is_array( $specs ) || count( $specs ) < $expected_specs || $expected_moq !== $moq || ! has_post_thumbnail( $product->ID ) ) {
			return false;
		}
	}
...
function custom_box_product_sample_deploy_batches(): array {
	return array(
		...
		array(
			'name'            => 'Rigid paperboard magazine file holder September 2026',
			'marker'          => 'product-samples-magazine-file-holder-202609',
			'expected'        => 1,
			'min_words'       => 1500,
			'expected_moq'    => '1000 sets',
			'expected_status' => 'publish',
			'always'          => true,
			'scripts'         => array(
				'tools/import-rigid-paperboard-magazine-file-holder-202609.php',
				'tools/verify-rigid-paperboard-magazine-file-holder-202609.php',
			),
		),
		array(
		...
		array(
			'name'            => 'Alibaba Christmas gift-box collection September 2026',
...
function custom_box_product_sample_deploy_selected_batches( string $scope ): array {
	...
	if ( 'magazine_file_holder_202609' === $scope ) {
		return array_values(
			array_filter(
				$batches,
				static function ( $batch ) {
					return isset( $batch['marker'] ) && 'product-samples-magazine-file-holder-202609' === $batch['marker'];
				}
			)
		);
	}
	...
	if ( 'christmas_collection_202609' === $scope ) {
		return array_values(
			array_filter(
				$batches,
				static function ( $batch ) {
					return isset( $batch['marker'] ) && 'alibaba-christmas-gift-box-collection-20260917' === $batch['marker'];
				}
			)
		);
	}
...
function custom_box_product_sample_deploy_run_next_step( array &$state ): void {
	...
	while ( isset( $batches[ (int) $state['batch_index'] ] ) ) {
		...
		if ( 0 === (int) $state['script_index'] ) {
			...
			if ( empty( $batch['always'] ) && custom_box_product_sample_deploy_batch_complete( $batch['marker'], $batch['expected'], $min_words, $batch['expected_moq'] ?? '1000 boxes', $batch['expected_status'] ?? 'publish', $batch['expected_specs'] ?? 21 ) ) {
...
function custom_box_product_sample_deploy_restore_tools() {
	...
	$latest_scripts = array(
		...
		'import-alibaba-christmas-gift-box-collection-20260917.php',
...
function custom_box_product_sample_deploy_page() {
	...
	<div class="wrap">
		...
		<p><strong>Current latest release:</strong> five Halloween paper bag products, five Halloween box products, the September 2026 rigid paperboard magazine file holder, and five Alibaba Christmas gift-box visualizations. Run <strong>Latest batch only</strong> after pulling the deployment branch.</p>
		...
		<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
			...
			<p>
				<label>
					<input type="radio" name="deploy_scope" value="christmas_collection_202609">
					Alibaba Christmas gift-box collection September 2026 only
				</label>
			</p>
			<?php wp_nonce_field( 'custom_box_product_sample_deploy' ); ?>
