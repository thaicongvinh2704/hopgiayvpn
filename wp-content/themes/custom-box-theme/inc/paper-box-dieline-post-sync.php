<?php
/**
 * Deploys the upgraded paper-box dieline draft and its image set.
 */
...
add_action('admin_notices', 'custom_box_paper_box_dieline_admin_notice');

function custom_box_paper_box_dieline_post_data(): array
{
    return array(
        ...
        'seo_title' => 'What Is a Dieline in Packaging? Cut, Crease & Bleed',
        'seo_description' => 'Learn what a packaging dieline is, how cut, crease, bleed and safe areas work, and how to check artwork before printing custom paper boxes.',
        'focus_keyword' => 'what is a dieline in packaging',
    );
}

function custom_box_paper_box_dieline_images(): array
{
    return array(
        'featured' => array(
            'base' => 'what-is-a-dieline-in-packaging-guide',
            'alt' => 'Packaging dieline beside an assembled folding carton showing cut and crease structure',
            'title' => 'What Is a Dieline in Packaging',
            'caption' => 'A flat dieline controls how a printed sheet becomes a finished folding carton.',
        ),
        'slot_1' => array(
            'base' => 'packaging-dieline-flat-template-and-box',
            'alt' => 'Flat folding carton dieline placed beside the assembled paper box',
            'title' => 'Flat Dieline and Finished Folding Carton',
            'caption' => 'The flat template identifies panels, flaps and folds before the carton is assembled.',
        ),
        'slot_2' => array(
            'base' => 'cut-crease-bleed-safe-area-packaging',
            'alt' => 'Diagram showing cut line crease line bleed and safe area on a carton panel',
            'title' => 'Cut Crease Bleed and Safe Area',
            'caption' => 'Each line controls a different production or artwork risk.',
        ),
        'slot_3' => array(
            'base' => 'folding-carton-artwork-crossing-crease-risk',
            'alt' => 'Folding carton samples comparing artwork across a crease with safer artwork placement',
            'title' => 'Artwork Placement Across a Carton Crease',
            'caption' => 'A structurally correct dieline can still produce a visible fold defect when artwork and finishing ignore the crease.',
        ),
        'slot_4' => array(
            'base' => 'packaging-dieline-prepress-check-workflow',
            'alt' => 'Prepress workstation checking packaging dieline layers artwork proof and folding carton sample',
            'title' => 'Packaging Dieline Prepress Check',
            'caption' => 'A production-ready file connects the approved structure, artwork proof and physical sample.',
        ),
    );
}

function custom_box_paper_box_dieline_content(): string
{
    $path = __DIR__ . '/post-content/what-is-a-paper-box-dieline.html';
...
function custom_box_sync_paper_box_dieline_post(): void
{
    ...
    if (
        ...
        && $post
        && custom_box_paper_box_dieline_is_complete((int) $post->ID)
    ) {
        return;
    }
    ...
    if (is_wp_error($post_id)) {
        delete_option(CUSTOM_BOX_PAPER_BOX_DIELINE_VERSION_OPTION);
        update_option(CUSTOM_BOX_PAPER_BOX_DIELINE_NOTICE_OPTION, array(
            'success' => false,
    ...
    if (custom_box_paper_box_dieline_is_complete((int) $post_id)) {
        update_option(
            CUSTOM_BOX_PAPER_BOX_DIELINE_VERSION_OPTION,
            CUSTOM_BOX_PAPER_BOX_DIELINE_SYNC_VERSION,
            false
        );
        update_option(CUSTOM_BOX_PAPER_BOX_DIELINE_NOTICE_OPTION, array(
            'success' => true,
            'message' => sprintf(
                'Packaging dieline draft synced: post ID %d, featured image %d, 4 inline figures, category Packaging Design / Printing Guide, 5 tags, and Rank Math fields verified.',
                (int) $post_id,
    ...
    update_option(CUSTOM_BOX_PAPER_BOX_DIELINE_NOTICE_OPTION, array(
        'success' => false,
        'message' => 'Packaging dieline sync is incomplete. Missing images: '
            . implode(', ', (array) get_option('custom_box_paper_box_dieline_missing_images', array()))
            . '; missing slots or validation failures: '
            . implode(', ', array_merge(
                (array) get_option('custom_box_paper_box_dieline_missing_slots', array()),
                (array) get_option('custom_box_paper_box_dieline_validation_failures', array())
            )),
    ), false);
}
...
function custom_box_upsert_paper_box_dieline_post()
{
    ...
    if ('' === trim($content)) {
        return new WP_Error(
            'paper_box_dieline_content_missing',
            'The canonical paper-box dieline content bundle is missing.'
        );
    }
    ...
    $post_id = (int) $result;
    custom_box_sync_paper_box_dieline_terms($post_id, $data);
    ...
    custom_box_sync_paper_box_dieline_images($post_id);

    return $post_id;
}

function custom_box_sync_paper_box_dieline_terms(int $post_id, array $data): void
{
...
function custom_box_sync_paper_box_dieline_images(int $post_id): void
{
    ...
    foreach (custom_box_paper_box_dieline_images() as $key => $image) {
        $attachment_id = custom_box_find_paper_box_dieline_attachment($image['base']);
        if (!$attachment_id) {
            $attachment_id = custom_box_create_paper_box_dieline_attachment(
                $image['base'],
                $post_id,
                $image
            );
        ...
        $marker = '<!-- paper-box-dieline-image:' . $key . ' -->';
        $figure = $marker
    ...
    update_option(
        'custom_box_paper_box_dieline_missing_images',
        array_values(array_unique($missing_images)),
        false
    );
    update_option(
        'custom_box_paper_box_dieline_missing_slots',
        array_values(array_unique($missing_slots)),
        false
    );
}

function custom_box_find_paper_box_dieline_attachment(string $base): int
{
    global $wpdb;
...
function custom_box_create_paper_box_dieline_attachment(
    string $base,
    int $post_id,
    array $image
): int {
...
function custom_box_paper_box_dieline_is_complete(int $post_id): bool
{
    $post = get_post($post_id);
    $data = custom_box_paper_box_dieline_post_data();
    $images = custom_box_paper_box_dieline_images();
    $failures = array();
    ...
    $content = $post ? (string) $post->post_content : '';
    if (
        4 !== substr_count($content, '<!-- paper-box-dieline-image:')
    ...
    foreach (array(
        'https://hopgiayvpn.com/products/folding-carton-boxes/',
        'https://hopgiayvpn.com/how-to-prepare-artwork-for-printed-paper-boxes/',
    ) as $required_link) {
    ...
    if ((array) get_option('custom_box_paper_box_dieline_missing_images', array())) {
        $failures[] = 'missing images';
    }
    if ((array) get_option('custom_box_paper_box_dieline_missing_slots', array())) {
        $failures[] = 'missing slots';
    }

    update_option(
        'custom_box_paper_box_dieline_validation_failures',
...
function custom_box_paper_box_dieline_admin_notice(): void
{
    if (!current_user_can('manage_options')) {
        return;
    }

    $notice = get_option(CUSTOM_BOX_PAPER_BOX_DIELINE_NOTICE_OPTION);
