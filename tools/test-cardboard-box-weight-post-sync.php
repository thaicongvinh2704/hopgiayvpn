<?php
/** Runs the cardboard box weight post sync and mandatory repair test locally. */

require_once dirname(__DIR__) . '/wp-load.php';
require_once dirname(__DIR__) . '/wp-content/themes/custom-box-theme/inc/cardboard-box-weight-post-sync.php';
...
$post_id = (int) $post_id;
if (!custom_box_cardboard_box_weight_is_complete($post_id)) {
    fwrite(STDERR, 'Normal sync failed: ' . wp_json_encode(get_option('custom_box_cardboard_box_weight_validation_failures')) . PHP_EOL);
    exit(1);
}
...
$damaged_content = preg_replace(
    '/<!-- cardboard-box-weight-image:slot_2 -->\s*<figure>.*?<\/figure>/is',
    '<!-- IMAGE_SLOT_2 -->',
...
custom_box_sync_cardboard_box_weight_post();
$repaired = custom_box_find_cardboard_box_weight_post(
    'how-much-does-a-cardboard-box-weigh',
    'How Much Does a Cardboard Box Weigh? A Practical Estimation Guide'
);
if (!$repaired || (int) $repaired->ID !== $post_id || !custom_box_cardboard_box_weight_is_complete($post_id)) {
    fwrite(STDERR, 'Repair failed: ' . wp_json_encode(get_option('custom_box_cardboard_box_weight_validation_failures')) . PHP_EOL);
    exit(1);
}
...
foreach (custom_box_cardboard_box_weight_images() as $image) {
...
echo wp_json_encode(array(
    ...
    'markers' => substr_count($content, '<!-- cardboard-box-weight-image:'),
