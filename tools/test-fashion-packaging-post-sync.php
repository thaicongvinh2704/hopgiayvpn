<?php
/** Runs the fashion packaging post sync and mandatory repair test locally. */
...
$post_id = (int) $post_id;
if (!custom_box_fashion_packaging_is_complete($post_id)) {
    fwrite(STDERR, 'Normal sync failed: ' . wp_json_encode(get_option('custom_box_fashion_packaging_validation_failures')) . PHP_EOL);
    exit(1);
}

update_option(CUSTOM_BOX_FASHION_PACKAGING_VERSION_OPTION, CUSTOM_BOX_FASHION_PACKAGING_SYNC_VERSION, false);
$post = get_post($post_id);
$damaged_content = preg_replace('/<!-- fashion-packaging-image:slot_2 -->\s*<figure>.*?<\/figure>/is', '<!-- IMAGE_SLOT_2 -->', (string) $post->post_content, 1);
...
custom_box_sync_fashion_packaging_post();
$repaired = custom_box_find_fashion_packaging_post('how-to-plan-fashion-product-packaging', 'How to Plan Paper Packaging for Fashion and Sportswear Products');
if (!$repaired || (int) $repaired->ID !== $post_id || !custom_box_fashion_packaging_is_complete($post_id)) {
    fwrite(STDERR, 'Repair failed: ' . wp_json_encode(get_option('custom_box_fashion_packaging_validation_failures')) . PHP_EOL);
    exit(1);
}
...
foreach (custom_box_fashion_packaging_images() as $image) {
    $attachment_ids[] = custom_box_find_fashion_packaging_attachment($image['base']);
    global $wpdb;
...
echo wp_json_encode(array(
    ...
    'markers' => substr_count($content, '<!-- fashion-packaging-image:'),
