<?php
/**
 * Verify magnetic closure box product import.
 *
 * Run:
 *   php tools/verify-magnetic-closure-products.php
 */
...
$expected = array(
    'custom-perfume-magnetic-closure-box' => 4,
    'custom-skincare-magnetic-closure-box' => 3,
    'custom-jewelry-magnetic-closure-box' => 4,
    'custom-watch-magnetic-presentation-box' => 4,
    'custom-electronics-magnetic-closure-box' => 4,
    'custom-candle-magnetic-gift-box' => 4,
    'custom-chocolate-magnetic-closure-box' => 4,
    'custom-apparel-magnetic-gift-box' => 4,
    'custom-corporate-pr-kit-magnetic-box' => 4,
    'custom-wine-magnetic-gift-box' => 4,
);
$failures = array();
...
foreach ($products as $product) {
    ...
    if ($figures < $min_images) {
        $failures[] = $title . ': missing expected inline product figures.';
    }
    if (!$featured) {
        $failures[] = $title . ': missing featured image.';
    }
    if (count($unique_images) < $min_images) {
        $failures[] = $title . ': has fewer than ' . $min_images . ' unique product images.';
    }
    if (is_wp_error($terms) || !in_array('magnetic-closure-boxes', $terms, true)) {
        $failures[] = $title . ': missing magnetic-closure-boxes category.';
    }
    if (is_array($specs)) {
...
echo 'Verified magnetic closure box products successfully.' . PHP_EOL;
