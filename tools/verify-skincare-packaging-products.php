<?php
/**
 * Verify skincare packaging product import.
 *
 * Run:
 *   php tools/verify-skincare-packaging-products.php
 */
...
foreach ($products as $product) {
    ...
    if (is_wp_error($terms) || !in_array('beauty-skincare-packaging', $terms, true)) {
        $failures[] = $title . ': missing beauty-skincare-packaging category.';
    }
    if (is_array($specs)) {
...
foreach ($expected as $slug) {
    ...
    if (!$found || $marker !== get_post_meta($found->ID, '_vpn_sample_import', true)) {
        $failures[] = 'Missing expected skincare product: ' . $slug . '.';
    }
}
...
echo 'Verified skincare packaging products successfully.' . PHP_EOL;
