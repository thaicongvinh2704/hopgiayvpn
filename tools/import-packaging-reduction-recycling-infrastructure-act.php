<?php
/**
 * Local CLI runner for the PRRIA post sync.
 *
 * Usage: php tools/import-packaging-reduction-recycling-infrastructure-act.php [import|audit|repair-test]
 */
...
$sync_file = get_template_directory() . '/inc/packaging-reduction-recycling-infrastructure-act-post-sync.php';
if (!function_exists('custom_box_sync_prria_post')) {
    require_once $sync_file;
}

function prria_import_report(): array
{
    ...
    foreach (custom_box_prria_images() as $key => $image) {
        $attachment_ids[$key] = custom_box_find_prria_attachment($image['base']);
    ...
    return array(
        'complete' => custom_box_prria_is_complete((int) $post->ID),
        'post_id' => (int) $post->ID,
        'status' => $post->post_status,
        'slug' => $post->post_name,
        'featured_id' => (int) get_post_thumbnail_id((int) $post->ID),
        'inline_markers' => substr_count($content, '<!-- packaging-reduction-recycling-infrastructure-act-image:slot_'),
        ...
        'validation_failures' => (array) get_option('custom_box_prria_validation_failures', array()),
        'sync_version' => (string) get_option(CUSTOM_BOX_PRRIA_VERSION_OPTION, ''),
    );
}
...
if ('import' === $mode) {
    custom_box_sync_prria_post();
    $report = prria_import_report();
} elseif ('audit' === $mode) {
    $report = prria_import_report();
} else {
    custom_box_sync_prria_post();
    $before = prria_import_report();
    ...
    $damaged = preg_replace(
        '/<!-- packaging-reduction-recycling-infrastructure-act-image:slot_2 -->\s*<figure\b.*?<\/figure>/is',
        '',
    ...
    $damaged_report = prria_import_report();
    custom_box_sync_prria_post();
    $after = prria_import_report();
