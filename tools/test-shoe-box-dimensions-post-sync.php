<?php
/** Runs the shoe-box dimensions sync and its mandatory local repair test. */

require_once dirname(__DIR__) . '/wp-load.php';
require_once dirname(__DIR__) . '/wp-content/themes/custom-box-theme/inc/shoe-box-dimensions-post-sync.php';
...
$original = custom_box_find_shoe_box_dimensions_post($data['slug'], $data['title']);
$original_id = $original ? (int) $original->ID : 0;

delete_option(CUSTOM_BOX_SHOE_BOX_DIMENSIONS_VERSION_OPTION);
custom_box_sync_shoe_box_dimensions_post();
$post = custom_box_find_shoe_box_dimensions_post($data['slug'], $data['title']);
if (!$post || !custom_box_shoe_box_dimensions_is_complete((int) $post->ID)) {
    fwrite(STDERR, 'Normal sync failed: ' . wp_json_encode(get_option(CUSTOM_BOX_SHOE_BOX_DIMENSIONS_VALIDATION_FAILURES_OPTION)) . PHP_EOL);
    exit(1);
}
...
foreach (custom_box_shoe_box_dimensions_images() as $image) {
    $before_attachment_ids[$image['base']] = custom_box_find_shoe_box_dimensions_attachment($image['base']);
}
...
$damaged_content = preg_replace(
    '/<!-- shoe-box-dimensions-image:slot_2 -->\s*<figure>.*?<\/figure>/is',
    '<!-- IMAGE_SLOT_2 -->',
...
if (custom_box_shoe_box_dimensions_is_complete($post_id)) {
    fwrite(STDERR, 'Damage phase did not make validation fail.' . PHP_EOL);
    exit(1);
}

custom_box_sync_shoe_box_dimensions_post();
$repaired = custom_box_find_shoe_box_dimensions_post($data['slug'], $data['title']);
if (!$repaired || (int) $repaired->ID !== $post_id || !custom_box_shoe_box_dimensions_is_complete($post_id)) {
    fwrite(STDERR, 'Repair failed: ' . wp_json_encode(get_option(CUSTOM_BOX_SHOE_BOX_DIMENSIONS_VALIDATION_FAILURES_OPTION)) . PHP_EOL);
    exit(1);
}
...
echo wp_json_encode(array(
    ...
    'markers' => substr_count($content, '<!-- shoe-box-dimensions-image:'),
