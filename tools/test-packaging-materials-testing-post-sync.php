<?php
/** Runs the packaging materials testing post sync and mandatory repair test locally. */

require_once dirname(__DIR__) . '/wp-load.php';
require_once dirname(__DIR__) . '/wp-content/themes/custom-box-theme/inc/packaging-materials-testing-post-sync.php';
...
$post_id = (int) $post_id;
if (!custom_box_packaging_materials_testing_is_complete($post_id)) {
    fwrite(STDERR, 'Normal sync failed: ' . wp_json_encode(get_option('custom_box_packaging_materials_testing_validation_failures')) . PHP_EOL);
    exit(1);
}
...
$damaged_content = preg_replace(
    '/<!-- packaging-materials-testing-image:slot_2 -->\s*<figure>.*?<\/figure>/is',
    '<!-- IMAGE_SLOT_2 -->',
...
custom_box_sync_packaging_materials_testing_post();
$repaired = custom_box_find_packaging_materials_testing_post(
    'testing-methods-packaging-materials',
    'Testing Methods for Packaging Materials: A Paper-Packaging Checklist'
);
if (!$repaired || (int) $repaired->ID !== $post_id || !custom_box_packaging_materials_testing_is_complete($post_id)) {
    fwrite(STDERR, 'Repair failed: ' . wp_json_encode(get_option('custom_box_packaging_materials_testing_validation_failures')) . PHP_EOL);
    exit(1);
}
...
foreach (custom_box_packaging_materials_testing_images() as $image) {
...
echo wp_json_encode(array(
    ...
    'markers' => substr_count($content, '<!-- packaging-materials-testing-image:'),
