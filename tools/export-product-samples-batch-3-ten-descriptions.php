...
$out   = array();
$out[] = '# Batch 3 Ten Product Descriptions Text Only';
$out[] = '';
...
foreach ( $products as $post ) {
	...
	$out[] = '- Word count without in-content images: ' . str_word_count( $text );
...
file_put_contents( dirname( __DIR__ ) . '/product-samples-batch-3-ten-descriptions-text-only.md', implode( PHP_EOL, $out ) );

echo 'Exported product-samples-batch-3-ten-descriptions-text-only.md' . PHP_EOL;
