<?php
/** Runs the E-flute thickness post sync and mandatory repair test locally. */

require_once dirname(__DIR__) . '/wp-load.php';
require_once dirname(__DIR__)
    . '/wp-content/themes/custom-box-theme/inc/e-flute-cardboard-thickness-post-sync.php';
...
$post_id = Custom_Box_E_Flute_Cardboard_Thickness_Post_Sync::upsert();
if (is_wp_error($post_id)) {
    fwrite(STDERR, $post_id->get_error_message() . PHP_EOL);
    exit(1);
}
$post_id = (int) $post_id;
if (!Custom_Box_E_Flute_Cardboard_Thickness_Post_Sync::is_complete($post_id)) {
    fwrite(
        STDERR,
        'Normal sync failed: '
            . wp_json_encode(
                get_option(Custom_Box_E_Flute_Cardboard_Thickness_Post_Sync::FAILURES_OPTION)
            )
            . PHP_EOL
    );
    exit(1);
...
update_option(
    Custom_Box_E_Flute_Cardboard_Thickness_Post_Sync::VERSION_OPTION,
    Custom_Box_E_Flute_Cardboard_Thickness_Post_Sync::VERSION,
    false
);
$post = get_post($post_id);
$damaged_content = preg_replace(
    '/<!-- e-flute-thickness-image:slot_2 -->\s*<figure>.*?<\/figure>/is',
    '<!-- IMAGE_SLOT_2 -->',
...
Custom_Box_E_Flute_Cardboard_Thickness_Post_Sync::run();
$repaired = Custom_Box_E_Flute_Cardboard_Thickness_Post_Sync::find_post();
if (
    !$repaired
    || (int) $repaired->ID !== $post_id
    || !Custom_Box_E_Flute_Cardboard_Thickness_Post_Sync::is_complete($post_id)
) {
    fwrite(
        STDERR,
        'Repair failed: '
...
foreach (Custom_Box_E_Flute_Cardboard_Thickness_Post_Sync::images() as $image) {
    $attachment_id = Custom_Box_E_Flute_Cardboard_Thickness_Post_Sync::find_attachment(
        $image['base']
    );
    ...
    $attachment_counts[$image['base']] = count(array_filter(
        $candidates,
        static function ($candidate_id) use ($image) {
...
$data = Custom_Box_E_Flute_Cardboard_Thickness_Post_Sync::data();
...
$duplicate_attachment_counts = array_filter(
    $attachment_counts,
...
echo wp_json_encode(array(
    ...
    'markers' => substr_count($content, '<!-- e-flute-thickness-image:'),
