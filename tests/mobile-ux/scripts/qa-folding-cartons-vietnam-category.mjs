...
const url =
  process.env.FOLDING_CARTONS_VIETNAM_CATEGORY_URL ||
  "http://localhost/hopgiayvpn/products/folding-carton-boxes/";
const screenshotDirectory = process.env.CATEGORY_QA_SCREENSHOT_DIR || "";
const expectedTitle =
  "Custom Folding Cartons Vietnam | Packaging Manufacturer";
const expectedDescription =
  "Source custom folding cartons in Vietnam for cosmetics, pharma, food and retail. Compare paperboard, structures, printing, samples and export packing.";
const viewports = [
...
try {
  for (const viewport of viewports) {
      ...
      ({ expectedTitleValue, expectedDescriptionValue }) => {
        ...
        return {
          ...
          guideCount: document.querySelectorAll(
            ".folding-carton-vietnam-guide",
          ).length,
          faqCount: document.querySelectorAll(
            ".folding-carton-vietnam-faq details",
          ).length,
    ...
    if (
      metrics.h1 !== "Custom Folding Cartons in Vietnam" ||
      metrics.h1Count !== 1
    ) {
    ...
    if (!metrics.heroAlt.toLowerCase().includes("folding carton")) {
    ...
    if (screenshotDirectory) {
      await page.screenshot({
        path: path.join(
          screenshotDirectory,
          `folding-cartons-vietnam-${viewport.name}.png`,
        ),
        fullPage: true,
        animations: "disabled",
      });
