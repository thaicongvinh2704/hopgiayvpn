...
const url =
  process.env.RIGID_BOX_MANUFACTURER_VIETNAM_CATEGORY_URL ||
  "http://localhost/hopgiayvpn/products/rigid-boxes/";
const screenshotDirectory = process.env.CATEGORY_QA_SCREENSHOT_DIR || "";
const expectedTitle =
  "Rigid Box Manufacturer Vietnam | Custom Luxury Boxes";
const expectedDescription =
  "Vietnam rigid box manufacturer for custom magnetic, drawer and lid-and-base boxes. Compare greyboard, wrapping, inserts, samples, MOQ and export packing.";
const viewports = [
...
try {
  for (const viewport of viewports) {
      ...
      ({ expectedTitleValue, expectedDescriptionValue }) => {
        ...
        return {
          ...
          guideCount: document.querySelectorAll(
            ".rigid-box-manufacturer-vietnam-guide",
          ).length,
          faqCount: document.querySelectorAll(
            ".rigid-box-manufacturer-vietnam-faq details",
          ).length,
    ...
    if (
      metrics.h1 !== "Rigid Box Manufacturer in Vietnam" ||
      metrics.h1Count !== 1
    ) {
    ...
    if (!metrics.heroAlt.toLowerCase().includes("rigid gift box")) {
    ...
    if (screenshotDirectory) {
      await page.screenshot({
        path: path.join(
          screenshotDirectory,
          `rigid-box-manufacturer-vietnam-${viewport.name}.png`,
        ),
        fullPage: true,
        animations: "disabled",
      });
